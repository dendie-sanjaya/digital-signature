...
Table of Contents
    ...
    Concept of File Hash

    Concept of Digital Signature

    Workflow for Creating & Verifying Digital Signatures
...
Signature Creation Workflow:
    ...
    Document Upload: The user uploads a document to the service.

    Key Generation (If Not Exists): If the user doesn't already have a key pair, the system will automatically generate a private and public key for them. The private key is stored securely in a separate file, while the public key is stored in the database.

    Document Hash Calculation: The system calculates the hash (digital fingerprint) of the uploaded document.

    Document Signing: This document hash is then signed using the user's private key. The result is the digital signature.

    Information Storage: The original document is saved in the server's storage (with a unique name), and all related information (original filename, unique filename, document hash, signer's public key, digital signature, user ID, publisher name) is stored in the database.

    QR Code: A QR code is generated containing a direct link to the digital signature information of that document.

Signature Verification Workflow:

    QR Code Scan: The party wishing to verify scans the QR code on the document.

    Signature Information Request: The QR code directs to the service's API, which retrieves all digital signature metadata from the database based on the document ID.

    Original Document Retrieval: The user can download the original document they wish to verify.

    Hash Recalculation: The verification system recalculates the hash of the downloaded document.

    Cryptographic Verification: The public key obtained from the API is used to verify the digital signature against the newly calculated document hash. If they match, the document is valid and has not been altered.
